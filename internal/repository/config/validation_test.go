...
func TestAccessors(t *testing.T) {
	...
	require.Equal(t, "[a-z][0-1]+", repository.Custom(cut).OwnerPermittedAliasRegex().String())
	require.Equal(t, "[a-z][0-2]+", repository.Custom(cut).OwnerProhibitedAliasRegex().String())
	require.Equal(t, "[a-z][0-3]+", repository.Custom(cut).OwnerFilterAliasRegex().String())
	require.Equal(t, "[a-z][0-4]+", repository.Custom(cut).ServicePermittedNameRegex().String())
	require.Equal(t, "[a-z][0-5]+", repository.Custom(cut).ServiceProhibitedNameRegex().String())
	require.Equal(t, "[a-z][0-6]+", repository.Custom(cut).RepositoryPermittedNameRegex().String())
	require.Equal(t, "[a-z][0-7]+", repository.Custom(cut).RepositoryProhibitedNameRegex().String())
