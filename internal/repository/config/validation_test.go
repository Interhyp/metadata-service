...
func TestAccessors(t *testing.T) {
	...
	require.Equal(t, "[a-z][0-1]+", repository.Custom(cut).OwnerAliasPermittedRegex().String())
	require.Equal(t, "[a-z][0-2]+", repository.Custom(cut).OwnerAliasProhibitedRegex().String())
	require.Equal(t, uint16(1), repository.Custom(cut).OwnerAliasMaxLength())
	require.Equal(t, "[a-z][0-3]+", repository.Custom(cut).OwnerFilterAliasRegex().String())
	require.Equal(t, "[a-z][0-4]+", repository.Custom(cut).ServiceNamePermittedRegex().String())
	require.Equal(t, "[a-z][0-5]+", repository.Custom(cut).ServiceNameProhibitedRegex().String())
	require.Equal(t, uint16(2), repository.Custom(cut).ServiceNameMaxLength())
	require.Equal(t, "[a-z][0-6]+", repository.Custom(cut).RepositoryNamePermittedRegex().String())
	require.Equal(t, "[a-z][0-7]+", repository.Custom(cut).RepositoryNameProhibitedRegex().String())
	require.Equal(t, uint16(3), repository.Custom(cut).RepositoryNameMaxLength())
	require.Equal(t, ";", repository.Custom(cut).RepositoryKeySeparator())
	require.Equal(t, []string{"some-type", "some-other-type"}, repository.Custom(cut).RepositoryTypes())
}
